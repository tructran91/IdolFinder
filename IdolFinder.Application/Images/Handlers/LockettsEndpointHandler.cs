...
{
    public class LockettsEndpointHandler : BaseEndpointHandler
    {
        public LockettsEndpointHandler(IStorageService storageService, IHttpClientFactory httpClientFactory)
        ...
        public override async Task<int> HandleAsync(string endpoint, CancellationToken cancellationToken)
        {
            ...
            foreach (var img in imgNodes)
            {
                ...
                try
                {
                    ...
                    await _storageService.SaveFileAsync(EndpointName.Locketts, fileName, imageBytes);
                    count++;
                }
                catch
                {
